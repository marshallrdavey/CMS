...
// This is a program to deform a vessel with a pressure wave applied to
// the inside of the vessel.
// A 2-dimensional implementation is not supported in this model.
// Function descriptions are included above the function definition.
...
{
public:
    vessel(const unsigned int fe_order, 
...
private:
    ...
    unsigned int fe_order;              // finite element order
    unsigned int n_global_refinements;  // number of global refinements on mesh
    
    // functions for mechanics
    double alpha;               // neo-Hookean material parameter (MPa)
    ...
    // body force
    const Vector<double> body_force{0., 0., 0.};
 
    // pressure
    double wave_function(const double x);
    double pressure_function(const double z);
    const double p_max = 0.005;  // max pressure (MPa)
    const double delay = 0.;     // wave onset delay (s)
    const double omega = 300.;   // wave speed (mm/s)
    const double nu    = 1.;     // frequency (1/s)
    const double p_a   = 0.1;    // wave shape parameter
    const double w_max = wave_function(p_a*std::log(p_a*(std::exp(1./p_a)-1.)));
    ...
    const double end_time = 2.; // final time (s)
    double time;
...
// class constructor
template <int dim>
vessel<dim>::vessel(const unsigned int fe_order,
                                  const unsigned int n_global_refinements,
                                  const double alpha)
    :fe_order(fe_order),
...
template <int dim>
double vessel<dim>::delta(unsigned int i,
                                 unsigned int j)
{
    return i == j ? 1.: 0.;
}
...
template <int dim>
double vessel<dim>::I_1(const Tensor<2, dim>& FF)
{
...
template <int dim>
Tensor<2, dim> vessel<dim>::PK1_stress(const Tensor<2, dim>& FF,
                                              const double& I1,
                                              const double& J)
{
...
template <int dim>
Tensor<4, dim> vessel<dim>::script_A(const Tensor<2, dim>& FF,
                                            const double& I1,
                                            const double& J)
{
    Tensor<4, dim> A;
...
// wave shape function
template <int dim> 
double vessel<dim>::wave_function(const double x)
{
    return x - (1. - std::exp(x/p_a))/(1. - exp(1./p_a));
}

// scaled wave shape function for max pressure and delay/frequency
template <int dim>
double vessel<dim>::pressure_function(const double z)
{
    if(z >= omega*time - delay || z <= omega*(time - 1./nu) - delay)
    {
        return 0.;
    }
    else
    {
        return wave_function(nu*((z+delay)/omega - (time-1./nu)))/w_max*p_max;
    }
}

// Generate vessel
// Vessel geometry generated in mm
template <int dim>
void vessel<dim>::setup_mesh()
{
    // create 2-dimensional mesh 
    // the creator of cylider_shell did not include a colorize option
    const Point<2> center(0., 0.);
    Triangulation<2> intermediate_mesh;
    GridGenerator::hyper_shell(intermediate_mesh,
                               center,  // center
                               4.5,     // inner radius (mm)
                               5.,      // outer radius (mm)
                               0,       // adaptive number of cells
                               true);   // colorize
    
    // extrude 2-dimensional mesh
    GridGenerator::extrude_triangulation(intermediate_mesh,
                                         20,            // number of slices
                                         20.,           // height (mm)
                                         triangulation, // final mesh  
                                         true);         // copy manifold ids
...
// dof handler stuff
// initialize the DoFs, the constraints, the vectors, and the matrices
template <int dim>
void vessel<dim>::setup_system()
{
    ...
    // constrain z displacement to 0 on inflow and outflow DoFs
    std::vector<bool> mask_vector{false, false, true};
    ComponentMask component_mask(mask_vector);
    constraints.clear();
...
template<int dim>
void vessel<dim>::assemble_mass_matrix()
{
...
// sum of internal stress force and external force
template<int dim>
void vessel<dim>::update_force()
{
    force    = 0;
    J_vector = 0;
    ...
    for (const auto &cell : dof_handler.active_cell_iterators())
    {
        ...
        {   
            ...
            if(face->at_boundary() && face->boundary_id() == 0)
            {
                ...
                {
                    const double pressure = pressure_function(fe_face_values.quadrature_point(f_q_index)[2]);
                    for(const unsigned int i : fe_values.dof_indices())
                    {
                        const unsigned int i_component = fe.system_to_component_index(i).first;
                        cell_rhs(i) -= pressure*
                                       fe_face_values.normal_vector(f_q_index)[i_component]*
...
template<int dim>
void vessel<dim>::initialize_acceleration()
{
...
// done at the onset of a time step and does not change for that time step
template<int dim>
void vessel<dim>::intermediate_step()
{
...
template<int dim>
void vessel<dim>::update_step()
{
    // update acceleration
    acceleration = 0;
    acceleration.add(1./beta/dt/dt, displacement);
    acceleration.add(-1./beta/dt/dt, displacement_tilde);
    
    // update velocity
    velocity = 0;
    velocity.add(1., velocity_tilde);
    velocity.add(gamma*dt, acceleration);

    // update the residual
    residual = 0;
    residual.add(1., force);
    const Vector<double> negative_acceleration = -1.*acceleration;
    unconstrained_mass_matrix.vmult_add(residual, negative_acceleration);    

    // constrain residual for solve
...
// matrix assembly
// Tangent stiffness matrix added to scaled mass matrix
template <int dim>
void vessel<dim>::assemble_system()
{
...
// matrix solve
// Newton solver, hence GMRES and Jacobi
// LU decomposition not used because of the large number of DoFs
template <int dim>
void vessel<dim>::solve()
{
    SolverControl            solver_control(1000, 1e-12);
...
// ouptut function
template <int dim>
void vessel<dim>::output_results(const unsigned int& step) const
{
